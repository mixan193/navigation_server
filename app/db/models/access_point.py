...
class AccessPoint(Base):
    ...
    is_mobile = Column(Boolean, nullable=False, server_default='false', comment="Мобильная точка доступа")

    created_at = Column(DateTime(timezone=True), server_default=func.now(), nullable=False)

    # ORM-связи
    building = relationship("Building", back_populates="access_points")
    observations = relationship("WiFiObs", back_populates="access_point", cascade="all, delete-orphan")
